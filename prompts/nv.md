...
- **Check against original vision** - does step output align with project specification?
...
### Validation Step Output Format

#### ASSESSMENT
```
PROCEED / REFINE / BACKTRACK
```

#### ISSUES
```
[Specific problems identified, if any]
```

#### RECOMMENDATION
```
[What should be done about identified issues]
```
...
# STEP {{MAIN_STEP_NUMBER}} OUTPUT TO VALIDATE

{{STEP_OUTPUT_TO_VALIDATE}}
...
Execute Step {{STEP_NUMBER}} validation for the above Step {{MAIN_STEP_NUMBER}} output. Check alignment with the project specification, logical consistency, and completeness. Use the structured validation output format and exercise draconian discipline in identifying any issues.
```